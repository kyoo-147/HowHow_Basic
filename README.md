...
The vNext Phase A/B/D1/D2/E1/F architecture and boundaries are in `docs/VNEXT_ARCHITECTURE.md`; the release evidence split is in `docs/RELEASE_VERIFICATION_MATRIX.md`. Phase E1 adds clean-room, hash-bound adapter envelopes and `integration doctor|contracts|export|import`. `integration doctor` only reads optional checkouts; a missing checkout is `AVAILABLE_CONTRACT_NOT_INSTALLED`, never a failure or live-use claim. Imports retain raw receipts and remain `PROVISIONAL` until schema, exact-pin, hash, and cross-link validation. AI-Scientist, DeerFlow, and other restricted/reference integrations cannot execute on the host. Phase D1 adds `paper context`, immutable `paper section add|list`, and `paper audit`: context snapshots are read-only, section imports only anchor existing claims, and substantive completeness is contract-based rather than page-based. The deterministic paper fixture is explicitly non-scientific. Phase B adds an immutable literature protocol, provisional candidate adapter, retained-source matrix, transformed-text provenance contract, and deterministic coverage/contradiction audit via `howhow literature ...`. It never claims novelty or complete coverage. The 13 exact integration pins are recorded in the project-local `.howhow/integration-manifest.json`; reference-only entries are never called live.

`experiment run` accepts a JSON specification containing `id`, `hypothesis`, `command` (an argument array; never a shell string), `code_revision`, `seed`, and optional `inputs`, `environment`, and `timeout_seconds`. Phase C additionally provides immutable `experiment proposal`, `experiment analysis`, and one-shot `experiment grant` records; execute a grant with `experiment run --grant <id>`. Available profiles are `RECORD_ONLY` and `TRUSTED_LOCAL`; the latter is explicitly trusted local execution, not a sandbox, and does not prevent host filesystem or network access. `OS_ISOLATED` and `CONTAINER_ISOLATED` are disabled contracts. It copies only declared project-relative regular-file inputs into a temporary working directory, supplies a reduced environment plus `HOWHOW_SEED`/`PYTHONHASHSEED`, caps timeout at 300 seconds, retains at most 1 MiB from each output stream, and immutably records success or failure. This is bounded execution, **not an OS security sandbox**: a hostile executable may still access the host filesystem or network. Run only trusted commands; use an external container or OS sandbox when hostile-code isolation is required.
...
Review findings are recorded separately with `review add`, audited with `review audit --strict`, and summarized with `review status`. D2 adds immutable `artifact`, `citation`, `issue`, and `policy` expert records: `paper status` exposes one next action without dumping schemas. Artifact manifests fail closed on input/script/output mutation; citations separate identity from claim support; issues preserve dissent and distinguish machine assistance from human scientific review; policy/license blockers prevent strict bundling. `verify --profile vnext-detailed` requires D1 substantive content plus all D2 record classes. `fixture` remains the legacy ClaimLedger/Phase-0 profile and is not a detailed research episode. The integrated vNext detailed fixture is synthetic and test-only (`tests/test_vnext_detailed_e2e.py`), with no novelty, correctness, human-review, publication-readiness, or submission claim. No submission is performed.
