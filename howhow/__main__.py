...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "source":
            ...
            mode_guidance = {'Manual': 'You approve each consequential step; no automatic execution is implied.', 'Hybrid': 'HowHow prepares bounded proposals; you must confirm consequential choices and issue grants.', 'Auto': 'Routine bounded records may be prepared, but safety, license, human approval, and publication gates are never bypassed.'}
            emit({"mode": args.mode, "mode_guidance": mode_guidance[args.mode], "next_action": "inspect the project status, then follow its single next action", "integration_count": len(sources.get("integrations", [])), "opinion": capability_list(root)["opinion"], "expert_escape_hatches": ["capability inspect", "capability cards", "plan", "literature audit"], "steps": ["inspect the project status", "follow the single next action", "confirm consequential choices when prompted", "audit evidence and claims before human scientific review"]})
        elif args.command == "continue":
            continuation = continue_project(root, Path(args.response_file) if args.response_file else None)
            continuation.update({"mode": args.mode, "mode_guidance": {'Manual': 'You approve each consequential step.', 'Hybrid': 'Review and confirm consequential choices; grants always require approval.', 'Auto': 'Routine bounded records only; safety and publication gates remain.'}[args.mode], "next_action": continuation.get("instruction") or continuation.get("message") or "inspect the project status", "expert_escape_hatches": ["capability inspect", "capability cards", "plan", "literature audit"]})
            emit(continuation)
