...
## Phase E1 ecosystem adapter contracts and doctor

E1 supplies one concrete, bounded contract for each of the 13 pinned repositories. Contracts name the exact pin, reviewed upstream surface paths, deterministic fixture hashes, operations, restrictions, and `NOT_CALLED` boundary. `integration export` emits a schema/hash-bound envelope and `integration import` validates the pin, envelope/payload hashes, and provisional state before retaining the raw receipt. No import can promote `VERIFIED`, scientific readiness, novelty, or publication. Randall is translated to the current ARIS contract with lineage. The wanshuiyin contract covers stage, dossier, evidence/reviewer receipt, experiment, claim, and wiki exchange; the other contracts cover their specified bounded phase/checkpoint, retrieval, QC, ledger, skill, rerank, and quest exchanges.

`integration doctor` is read-only. It checks optional project/user/cache checkouts, exact Git HEAD, required surface presence, license-file status, platform/runtime and minimal dependency readiness. Missing checkouts report `AVAILABLE_CONTRACT_NOT_INSTALLED`, not failure and not live use. No checkout is cloned, changed, imported, executed, or used to fetch. AI-Scientist requires restricted-use acknowledgement plus manuscript AI disclosure; DeerFlow is optional isolated-profile-only; SciAgent remains reference-only pending an operative license; OpenScholar is offline retained-passage-only with Contriever CC-BY-NC boundary; DeepScientist has explicit Windows limitations. Capability cards expose operations, installed/enabled/live state, restrictions, and recommended use.

## Phase F integrated verification closure

`tests/test_vnext_detailed_e2e.py` starts from an empty temporary directory and drives the public API/CLI surfaces through the complete synthetic path: empty opinion, confirmations, 3-gated ideas, target, literature receipts and matrix, preserved baseline/one-change records, claims, frozen context, anchored sections, D2 provenance/review/policy records, all 13 provisional adapter receipts, strict LaTeX/package rebuild, and late-record tamper rejection. The fixture is impossible to interpret as a scientific result: it records fixture-only sources/results and explicitly makes no novelty, correctness, human-review, publication-readiness, or submission claim. `READY_FOR_HUMAN_REVIEW` remains a product package gate only. See `docs/RELEASE_VERIFICATION_MATRIX.md`.
